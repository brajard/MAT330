...
\title[Machine Learning]{Machine learning and (geo)physical modelling-1}
...
\begin{frame}{Overview of the next lectures}
    \begin{enumerate}
        \item Lecture 1 (Tuesday 21 Oct. 2023):\\ Generalities and Principles of Machine Learning
        \item Lecture 2 (Wednesday 23 Oct. 2023):\\ Neural networks, deep learning, training
        \item Lecture 3 (Tuesday 30 Oct. 2023):\\ Practical work
...
\begin{frame}[shrink=20]{References}
    ...
    \nocite{chollet2021deep,VanderPlas-2016, Goodfellow-et-al-2016, zhang2023dive}
...
\begin{figure}
\includegraphics[width=1.\textwidth] {fig/L1/progress-ml.png}
\end{figure}
\end{frame}
...
%%%%%%%%%%%%%%%%%%%%
\begin{frame}
\frametitle{Example 2: Playing Games}
\begin{columns}
\column{.65\textwidth}
\begin{itemize}
...
%  \begin{figure}
%        \centering
%        \includegraphics[width=0.45\textwidth]{fig/L1/AlphaFo%ld-Blog-9c_diagram2.png}
...
%%%%%%%%%%%%%%%%%%%%
\begin{frame}
\frametitle{Example 3: Text to image}
Objective: Generate an image from a textual description.
\begin{itemize}
\item {\bf 2021}: DALL$\cdot$E
\item {\bf 2022}: Midjourney and Stable diffusion
\item Since {\bf 2022}: all over the place

\end{itemize}
\pause
    \begin{figure}
        \centering
        \includegraphics[width=0.4\textwidth]{fig/L1/text2image.png}\\
photography style with a 22mm lens of a handsome redheaded biker man with a short beard.\end{figure}
\end{frame}


%%%%%%%%%%%%%%%%%%%%
\begin{frame}
\frametitle{Text to image - Progress in free tools}
A test on: {\small \url{https://deepai.org/machine-learning-model/text2img}}\\
\vspace{1em}
Prompt: \emph{``students attending a machine learning course''}
    \pause
\begin{columns}
    \column{.33\textwidth}
    \begin{figure}
        \centering
        Result in Oct. 2022
        \includegraphics[width=\textwidth]{fig/L1/person_text2image.jpeg}
    \end{figure}
...
\begin{figure}
    \centering
    Result in Oct. 2023
    \includegraphics[width=\textwidth]{fig/L1/generated_students_2023.jpeg}
\end{figure}
...
\begin{figure}
    \centering
    Result in Oct. 2025
    \includegraphics[width=\textwidth]{fig/L1/text2image_2025.jpeg}
\end{figure}
...
%%%%%%%%%%%%%%%%%%%%
\begin{frame}
\frametitle{Example 4: Large language models (LLM) }
Examples: GPT-5 (OpenAI), LLaMA 4(Meta AI), Gemini 2.5 (Google DeepMind), DeepSeek3.2 (DeepSeek).
    ...
    \begin{figure}
        ...
        \includegraphics[width=0.7\textwidth]{fig/L1/outline_chatbot.png}\\
\end{figure}
\end{frame}
...
\begin{frame}[t]{Reasons for these recent achievements?}
    \begin{itemize}
        ...
        \only<1> {
        \begin{figure}
\includegraphics[width=.8\textwidth]{fig/L1/Wikipedia_article_size_in_gigabytes.png}
\end{figure}
\rref[source: wikimedia.org]
}
        \item <2-> Progress in computing resources.
        \only<2> {
        \begin{figure}
\includegraphics[width=.4\textwidth]{fig/L1/evolution-gpu.png}
\end{figure}
\rref[source: NVDIA]
}        
        ...
        \item <3-> Scientific research on new algorithms (e.g adapted to image processing)
                \only<3> {
        \begin{figure}
\includegraphics[width=.7\textwidth]{fig/L1/diffusion_model.png}
\end{figure}
...
\begin{center}
    \begin{tabular}{cccc}
        \includegraphics[width=.16\textwidth]{fig/L1/TensorFlowLogo.png} &  
         \includegraphics[width=.18\textwidth]{fig/L1/keraslogo.png}& 
        \includegraphics[width=.18\textwidth]{fig/L1/pytorchlogo.png}&
         \includegraphics[width=.24\textwidth]{fig/L1/lightninglogo.png}\\
    \end{tabular}
\end{center}
}
...
%%%%%%%%%%%%%%%%%%%%
\begin{frame}{AI in physical sciences}
\begin{figure}
\includegraphics[width=.6\textwidth]{fig/L1/progress-mlinphysics.jpeg}
\end{figure}
\end{frame}
...
\begin{frame}{What about data assimilation?}
 ...
 { \footnotesize   
 \begin{itemize}
    \item Cheng, Sibo, et al., 2023. Machine learning with data assimilation and uncertainty quantification for dynamical systems: a review. {\it IEEE/CAA Journal of Automatica Sinica} 10.6:1361-1387.
...
\item From disorganized data to organized information
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}
\frametitle{Two types of settings}
...
\begin{itemize}
\item Determine typical behaviours of clients in a supermarket knowing what they have bought.
\end{itemize}
\item \alert{Self-Supervised Learning}: The training is supervised by the data itself
\begin{itemize}
...
%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}
\frametitle{Two classes of (self)-supervised learning problems}
\begin{enumerate}
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item $y$ is a subset of $x$: self-supervised learning
    \end{itemize}
...
\begin{itemize}
    ...
    \begin{itemize}
    \item some values of $y$ are known: supervised learning
    \item $y$ is quantitative: regression
    \end{itemize}
\pause
...
\begin{itemize}
    \item drastically reduces the number of samples that can be used for learning the model
...
\begin{enumerate}[<+->]
    ...
    \item To evaluate independently the performance of our model, we should compute the score on a \alert{third independent dataset: The test dataset}.
...
\begin{frame}{In summary}
    ...
    \begin{itemize}
        \item A test dataset (independent from the two others) to estimate the final performance of the model.
    \end{itemize}
\end{frame}
...
\begin{frame}{Additional Comments}
    \begin{figure}
        ...
        \begin{tikzpicture}[auto, node distance=1cm and 2cm]
            \node [block] (evaluate) {Evaluate model};
        \end{tikzpicture}
    \end{figure}

    \alert{Model evaluation is a critical step in the machine learning pipeline. Key considerations include:}

    \begin{itemize}
        \item Is the dataset and evaluation metric unbiased? Do they accurately reflect the desired model performance?
        \item Is there a simpler model that performs sufficiently well? Avoid proposing complex machine learning solutions when a simpler approach is more effective.
    \end{itemize}
\end{frame}

\begin{frame}{Adequate metric}
A typical metric for classification problems is the {\bf accuracy}:
$$
\mathrm{acc}=\frac{TP+TN}{T+N}$$\\
\begin{footnotesize}
...
\end{footnotesize}
\pause
\begin{block}{Example of Credit Card Fraud Detection}
\alert{Objective}: Predict if a credit card operation is fraudulent \\
In the dataset, frauds (positive class) are  {\bf 0.172\%} of the cases.
{\tiny \url{https://www.kaggle.com/datasets/mlg-ulb/creditcardfraud}}\\
\end{block}
\pause
Predicting always the negative class: acc={\bf 99.828\%} !!!\\
\alert{Accuracy is not a adequat metric in this case}\\
\pause
Other metric: $$\mathrm{recall}=\frac{TP}{P}$$
\end{frame}



\begin{frame}{Always define a baseline}
\begin{block}{Example: Weather Forecast}
Imagine we have a model predicting "rain" or "no-rain" the day after with an accuracy of 75\%.\\
\alert{Is it good?}
\end{block}
\pause
   \begin{figure}
    \includegraphics[width=.6\textwidth]{fig/L1/yr_snapshot.png}
    \end{figure}
\alert{A very simple baseline: the persistence.}\\
Predict that the day after is the same as the day before\\
\pause
Accuracy of persistence: 80\%\\
\begin{center}
     \alert{In this case, a ML model should do better than 80\% to be considered.}
\end{center}
...
\begin{frame}
      \begin{figure}
    \includegraphics[width=1.\textwidth]{fig/L1/kahoot.jpeg}
...
\begin{frame}{Structure of the features}
    \begin{block}{What is a feature?}
        It is a measurable property of the data.
    \end{block}
    Orgnisation of the features:
    
    \begin{tabular}{p{.2\textwidth}|p{.22\textwidth}|p{.2\textwidth}|p{.2\textwidth}}
        \alert{Scalar} & \alert{Image/Tensor} & \alert{Sequence} & \alert{Unstructured}\\
        {\footnotesize number of room of a house, profession, ...} &
        {\footnotesize  values of a pixel in a image} &
        {\footnotesize time series, text} &
        {\footnotesize available paths in a city map}
    \end{tabular}
\end{frame}

\begin{frame}{Type of features}
    \begin{itemize}[<+->]
        \item quantitative/continuous features (e.g. distance to employment centers, temperature)
        \item ordinal/discrete features (e.g. number of rooms, category of a hurricane)
        \item categorical features (e.g. name of the neighborhood, name of the ocean)
    \end{itemize}
    \pause
...
\begin{itemize}
    ...
    \item If the number of modalities (number of different features) is high, encoding qualitative feature produces a big-sized vector.
\end{itemize}
\end{frame}
...
%On the introduction of the \textit{Evensen et al.} book.\\
%${\rm v}$ is 3-dimensional ($x$,$y$,size)
%\begin{figure}
%    \includegraphics[trim={2cm 28cm 2cm 28cm},clip,width=.75\textwidth]{fig/L1/wordcloud.png}
%\end{figure}
%{\footnotesize
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}{Embedding for Large Language Models (LLM)}

\begin{figure}
    \centering
    \includegraphics[width=\linewidth]{fig/L1/embedding_matrix.png}
\end{figure}
\pause
\begin{columns}
    \column{.5\textwidth}
    \begin{figure}
        \centering
    \includegraphics[width=1.\linewidth]{fig/L1/embedding_process.png}
\end{figure}
\pause
    \column{.5\textwidth}
    Embeddings of similar words are close:
        \begin{figure}
        \centering
    \includegraphics[width=1.\linewidth]{fig/L1/tower_trimed.png}
\end{figure}
\end{columns}
\rref[https://www.3blue1brown.com/lessons/gpt]


\end{frame}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}{The notion of direction}

\begin{figure}
\centering
\only<1>{
\includegraphics[width=.6\linewidth]{fig/L1/direction1.png}
}
\only<2>{
\includegraphics[width=.6\linewidth]{fig/L1/direction2.png}
}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}{The space of embedding encodes some "meaning"}


        \begin{figure}
        \centering
    \includegraphics[width=.8\linewidth]{fig/L1/queen.png}
\end{figure}
...
\pause
        ...
        \begin{figure}
        \centering
    \includegraphics[width=.9\linewidth]{fig/L1/gender.png}
\end{figure}

\rref[https://www.3blue1brown.com/lessons/gpt]
\hfill More in the next lesson.
...
\begin{block}{}
When there are too many parameters to fit, the model can reproduce 
...
\item Elastic Net combines both regularizations
\end{itemize}
...
\begin{itemize}
\item Provides sparse solutions and reduces the dimension of the
predictor
...
It comes with an extra hyperparameter to tune (using, e.g., cross-validation): $\alpha$
\end{alertblock}
\end{frame}
...
\begin{frame}{An example using scikit-learn}
    \begin{figure}
    \caption*{California House Pricing}
...
%\begin{frame}[fragile]{Questions addressed in this lecture}
  %  \begin{itemize}
  %      \item What are the more common areas of application of machine learning? \cite[1]{chollet2021deep}
   %     \item What are the different classes of ML problems? \cite[5.1]{VanderPlas-2016}
  ...
  %      \item How to encode qualitative features? \cite[5.4]{VanderPlas-2016}
   %     \item What is L1(Lasso)/L2(Ridge) regularization? \cite[5.6]{VanderPlas-2016};\cite[4.4]{chollet2021deep}
...
%\cite[$n$]{chollet2021deep}: Chollet, \textit{Deep Learning with Python}, chapter $n$
%\end{block}
%\end{footnotesize}
%\end{frame}
